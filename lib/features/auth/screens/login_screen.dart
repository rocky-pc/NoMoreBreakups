...
class _LoginScreenState extends ConsumerState<LoginScreen>
  ...
  void initState() {
    ...
    _blob2 = Tween<double>(begin: 10, end: -16).animate(
      CurvedAnimation(
        parent: _blobController,
        curve: const Interval(0.2, 1.0, curve: Curves.easeInOutSine),
      ),
    );
    _blob3 = Tween<double>(begin: -8, end: 18).animate(
      CurvedAnimation(
        parent: _blobController,
  ...
  // ---------------------------------------------------------------------
  // Brand header (icon + title + tagline)
  // ---------------------------------------------------------------------
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                // Login button with soft gradient
  ...
  // ---------------------------------------------------------------------
  // Soft floating gradient blobs (same language as FeedScreen)
  // ---------------------------------------------------------------------
          ...
          builder: (context, _) {
            return Stack(
              children: [
                ...
                Positioned(
                  top: 30 + _blob2.value,
                  right: -90 + _blob2.value * 0.5,
                  child: _blob(200, AppColors.rose.withAlpha(isDarkMode ? 40 : 28)),
                ),

                // Blob 3 – bottom
                Positioned(
                  bottom: -80 + _blob3.value,
                  left: -50 + _blob3.value * 0.4,
                  child: _blob(220, AppColors.rose.withAlpha(isDarkMode ? 35 : 22)),
                ),

                // Extra soft mid blob
                Positioned(
                  top: MediaQuery.of(context).size.height * 0.45 + _blob1.value * 0.3,
                  right: -40,
                  child: _blob(160, AppColors.rose.withAlpha(isDarkMode ? 25 : 16)),
                ),

                // Heavy blur for that fluid wash
                Positioned.fill(
                  child: BackdropFilter(
